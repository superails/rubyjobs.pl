...
namespace :rails do
  ...
  task :console => 'rvm:hook' do
    on roles(:app), :primary => true do |host|
      execute_interactively host, "console -e #{fetch(:stage)}"  
    end
  end
  ...
  task :dbconsole => 'rvm:hook' do
    on roles(:app), :primary => true do |host|
      execute_interactively host, "dbconsole -e #{fetch(:stage)}"  
    end
  end
  ...
  def execute_interactively(host, command)
    ...
    exec "ssh -l #{fetch(:user)} #{host.hostname} -p #{host.port || 22} -t '#{command}'"
  end
end
